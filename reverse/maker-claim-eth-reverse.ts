...
import type { BTCEscrowDst, IBaseEscrow } from "../evm/typechain-types";
...
async function main() {
  ...
  console.log("======================================================");
  console.log("💡 MAKER: Claiming ETH from EVM escrow reveals the secret!");
  ...
  // Get the BTCEscrowDst contract (for BTC→EVM reverse flow)
  const BTCEscrowDstFactory = await ethers.getContractFactory("BTCEscrowDst");
  const escrow = BTCEscrowDstFactory.attach(order.evmEscrow.address) as BTCEscrowDst;
  ...
  try {
    ...
    console.log("🔓 Secret revealed on EVM blockchain!");
    
    console.log("\n🔥 CRITICAL: SECRET IS NOW PUBLIC!");
    console.log("===================================");
    console.log("🔓 Secret:", order.secret);
    console.log("📡 Visible on EVM blockchain in transaction:", tx.hash);
    ...
    console.log("\n🎯 NEXT STEP FOR TAKER:");
    console.log("=======================");
    console.log("🔓 Secret is now public on EVM blockchain!");
    console.log("🔸 TAKER can extract secret and claim BTC:");
    console.log("   ORDER_ID=" + orderId + " npm run reverse:taker:claim");
    console.log("🔸 TAKER just needs to extract secret from EVM TX:", tx.hash);

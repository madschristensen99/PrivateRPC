...
import type { BTCEscrowFactory, BTCEscrowDst } from "../evm/typechain-types";
...
async function main() {
  console.log("🔄 TAKER: CREATING EVM ESCROW (REVERSE FLOW)");
  console.log("============================================");
  console.log("💡 TAKER: Creating EVM escrow with ETH for BTC→ETH swap");
  ...
  console.log("👤 TAKER (EVM):", taker.address);
  ...
  const immutables = {
    ...
    maker: BigInt(order.maker.address),      // MAKER (EVM address)
    taker: BigInt(taker.address),            // TAKER (EVM address) 
  ...
  console.log("\n🔨 Creating EVM Destination Escrow (BTCEscrowDst)...");
  console.log("====================================================");
  ...
  try {
    ...
    // Update order with EVM escrow info and taker details
    order.evmEscrow = {

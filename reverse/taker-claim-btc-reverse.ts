...
async function main() {
  ...
  console.log("=============================================================");
  console.log("💡 TAKER: Extracting secret from EVM transaction and claiming BTC!");
  ...
  if (!order.taker || !order.evmEscrow) {
    throw new Error("❌ Order missing taker or EVM escrow info");
  }
  ...
  console.log("🔸 EVM claim TX:", order.transactions.evmEscrowClaim);

  console.log("\n🔍 EXTRACTING SECRET FROM EVM BLOCKCHAIN:");
  console.log("==========================================");
  console.log("📡 Checking EVM transaction:", order.transactions.evmEscrowClaim);
  console.log("🌐 EVM network: Sepolia");
  console.log("🔍 Explorer URL: https://sepolia.etherscan.io");
  console.log("🔍 Looking for secret in transaction input data...");
  
  // Get transaction details from EVM blockchain
  const evmTx = await ethers.provider.getTransaction(order.transactions.evmEscrowClaim);
  if (!evmTx) {
    throw new Error(`❌ Could not find EVM transaction: ${order.transactions.evmEscrowClaim}`);
  }
  
  console.log("📋 Transaction data length:", evmTx.data.length);
  console.log("📋 Transaction data (first 100 chars):", evmTx.data.substring(0, 100) + "...");
  ...
  if (evmTx.data.length >= 74) { // 4 bytes selector + 32 bytes secret = 36 bytes = 72 hex chars + 0x
    // Skip function selector (4 bytes = 8 hex chars) and extract secret (32 bytes = 64 hex chars)
    extractedSecret = "0x" + evmTx.data.substring(10, 74);
  } else {
    throw new Error("❌ Could not extract secret from EVM transaction data");
  }
  
  console.log("✅ SECRET EXTRACTED FROM EVM BLOCKCHAIN!");
  console.log("=========================================");
  ...
  try {
    ...
    if (claimSuccessful && txId) {
      ...
      console.log("📡 Secret was extracted from EVM blockchain transaction!");
      ...
      console.log("✅ Step 2: TAKER created EVM escrow with ETH");
      ...
      console.log("🔸 Secret was revealed on EVM blockchain FIRST!");
      ...
      console.log("================================");
      console.log("✅ Secret successfully extracted from EVM blockchain");
